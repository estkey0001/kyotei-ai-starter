...
class KyoteiDataManager:
    ...
    def __init__(self):
        self.data_dir = "/mnt/aidrive/kyotei_v14_real_data/data/coconala_2024"
        self.db_path = "/home/user/kyotei_racer_master.db"
        self.venues = {
            'toda': '戸田',
            'edogawa': '江戸川', 
            'heiwajima': '平和島',
    ...
    def verify_data_sources(self):
        ...
        # データベースチェック
        db_status = "✅" if os.path.exists(self.db_path) else "❌"
        print(f"{db_status} データベース: {self.db_path}")
        if os.path.exists(self.db_path):
            try:
                ...
                print(f"  └ 選手データ: {count:,}件")
                conn.close()
            except Exception as e:
                print(f"  └ エラー: {e}")

        # データディレクトリチェック
        dir_status = "✅" if os.path.exists(self.data_dir) else "❌"
        print(f"{dir_status} データディレクトリ: {self.data_dir}")

        if os.path.exists(self.data_dir):
            csv_files = []
            for venue_code in ["edogawa", "heiwajima", "omura", "suminoe", "toda"]:
                csv_path = f"{self.data_dir}/{venue_code}_2024.csv"
                if os.path.exists(csv_path):
                    size = os.path.getsize(csv_path)
                    csv_files.append((venue_code, size))
                    print(f"  ✅ {venue_code}_2024.csv ({size:,} bytes)")
                else:
                    print(f"  ❌ {venue_code}_2024.csv (見つからない)")

            print(f"  └ 利用可能CSVファイル: {len(csv_files)}/5")
        ...
        for venue_code, venue_name in venue_names.items():
            ...
            try:
                if os.path.exists(filename):
                    ...
                    self.race_data[venue_code] = df
                    loaded_venues.append(f"{venue_name}({len(df)}件)")
                    print(f"  ✅ {venue_name}: {len(df):,}件のレースデータ")
                else:
                    print(f"  ❌ {venue_name}: ファイルが見つかりません")

            except Exception as e:
                print(f"  ❌ {venue_name}: 読み込みエラー - {e}")

        if loaded_venues:
            print(f"\n✅ データ読み込み完了: {len(loaded_venues)}会場")
            print(f"   読み込み済み: {', '.join(loaded_venues)}")

            # デフォルト表示データを設定
            self._setup_default_display_data()
            return True
        else:
            print("\n❌ データ読み込み失敗: 利用可能なデータがありません")
            return False

    def _setup_default_display_data(self):
        """デフォルト表示用のサンプルデータを設定"""
        if not self.race_data:
            return

        # 最初に利用可能な会場のデータを使用
        first_venue = list(self.race_data.keys())[0]
        sample_df = self.race_data[first_venue]

        # 最新の日付を取得
        if 'date' in sample_df.columns:
            latest_date = sample_df['date'].max()
            print(f"\n📅 デフォルト表示日: {latest_date}")

            # その日のレースデータを取得
            daily_races = sample_df[sample_df['date'] == latest_date]

            if len(daily_races) > 0:
                # レース番号でグループ化
                race_groups = daily_races.groupby('race_no')
                sample_race_no = race_groups.groups.__iter__().__next__()
                sample_race_data = race_groups.get_group(sample_race_no)

                print(f"📊 サンプルレース: {first_venue.upper()} {latest_date} 第{sample_race_no}R")
                print(f"   参加選手: {len(sample_race_data)}名")

                # サンプルレースの詳細表示
                for _, row in sample_race_data.iterrows():
                    print(f"   {int(row['pit_no'])}号艇: {row['racer_name']} (登録番号: {int(row['racer_id'])})")
